# sign2gpt/Sign2GPT/trainer/psuedo_gloss_trainer.py
...
class Trainer(BaseTrainer):
    """
    Updated for DINOv3 + LoRA integration.

    Key fixes/improvements:
    - safer yaml saving via yaml.safe_dump
    - autocast device_type works on CUDA/CPU
    - auto_model sync_bn only when distributed > 1 (avoids unnecessary SyncBN conversion)
    - optional find_unused_parameters via cfg (useful if experimenting with selective adapters)
    ...
    def __init__(self, local_rank, *args, **kwargs):
        ...
        if "run" in cfg:
            ...
            if getattr(cfg, "log_name", None):
                stage_name = (
                    "pretraining_" + cfg.log_name + "_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                )
            else:
        ...
        self.scheduler = self.prep_scheduler(cfg, train_dl, self.optimizer, trainer, evaluator)
        ...
        self.save_checkpoints(cfg, trainer, evaluator, score_function, best_only=False, to_save=to_save)
    ...
    # runner hooks
    # -------------------------
    def prepare_runner(self, cfg, trainer, evaluator, valid_dl, valid_tester, test_tester, test_dl):
    ...
    def dict_metric_from_list(self, engine_type, list_of_metrics, dict_metrics, **kwargs):
        ...
        if "accuracy" in list_of_metrics:
            ...
            dict_metrics[f"{engine_type}/avg_acc"] = AccuracyScore(output_transform=output_fn, thresholds=0.5)
        ...
        if "f1_score" in list_of_metrics:
            ...
            dict_metrics[f"{engine_type}/f1_score"] = F1Score(output_transform=output_fn, thresholds=0.5)
    ...
    def init_metrics(self, engine, engine_type, list_of_metrics, additional=True, **kwargs):
        dict_metrics = {}
        dict_metrics = self.dict_metric_from_list(engine_type, list_of_metrics, dict_metrics, **kwargs)
        super().init_metrics(engine, engine_type, dict_metrics=dict_metrics, additional=additional)
    ...
    def init_models(self):
        ...
        # Useful if experimenting and you hit "unused parameter" DDP errors:
        find_unused = bool(getattr(self.cfg, "find_unused_parameters", False))

        # SyncBN only makes sense in multi-GPU runs and when BN layers exist.
        sync_bn = bool(getattr(self.cfg, "sync_bn", True)) and (idist.get_world_size() > 1)

        self.model = idist.auto_model(self.model, find_unused_parameters=find_unused, sync_bn=sync_bn)
    ...
    def prep_batch(self, batch, isValid=False, cuda=True):
        ...
        res = {
            "model_input": {
                ...
                "max_len": torch.tensor(self.cfg.max_seq_len if "max_seq_len" in self.cfg else 512),
            },
            "targets": {
                "pseudo_gloss_ids": batch["pseudo_gloss_ids"] if "pseudo_gloss_ids" in batch else [],
                "index": torch.stack(idx),
            },
        }

        return convert_tensor(res, device=idist.device(), non_blocking=True) if cuda else res
    ...
    def train_step(self, engine, batch):
        ...
        return {
            "y_pred": y_pred,
            "target": x,
            "losses": {"loss": loss.detach(), **{k: v.detach() for k, v in dict_losses.items()}},
        }

    def manually_update_gradients(self):
        # hook for custom gradient masking/surgery if needed
        pass
