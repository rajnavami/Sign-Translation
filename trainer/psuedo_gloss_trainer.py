...
class Trainer(BaseTrainer):
    def __init__(self, local_rank, *args, **kwargs):
        ...
        # ── Optimizer with separate LR for flow params ────────────────────────
        flow_params = []
        other_params = []
        ...
        to_save = {"model": self.model, "optimizer": self.optimizer, "trainer": trainer}
        ...
        objects_to_load = {"model": self.model, "optimizer": self.optimizer, "trainer": trainer}
    ...
    def dict_metric_from_list(self, engine_type, list_of_metrics, dict_metrics, **kwargs):
        ...
        if "class_accuracy" in list_of_metrics:
            ...
            dict_metrics[f"{engine_type}/cls_acc"] = ClassAccuracyScore(
                output_transform=output_fn, thresholds=0.5, num_classes=kwargs["num_classes"])
    ...
    def init_models(self):
        ...
        self.model = idist.auto_model(self.model, find_unused_parameters=False, sync_bn=True)

    def prep_batch(self, batch, isValid=False, cuda=True):
        """
        FIX: flow_data popped BEFORE convert_tensor, moved to GPU manually,
        then reattached. convert_tensor crashes on None values inside dicts.
        """
        idx, frames = (batch["index"], batch["frames"])
        frame_features = frames

        # Extract flow BEFORE building res — kept out of convert_tensor
        ...
        res = {
            "model_input": {
                ...
                # flow_data intentionally excluded here — added back after convert_tensor
            },
            "targets": {
