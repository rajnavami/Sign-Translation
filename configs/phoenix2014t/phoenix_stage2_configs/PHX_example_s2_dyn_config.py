...
def get_config():
    ...
    cfg.log_name  = "dinov3_optical_flow_rank16_alllayers_v1"
    ...
    # ── Stage 1 checkpoint ────────────────────────────────────────────────────
    # Used by base_trainer.load_checkpoints to initialise the sign encoder.
    # Points directly to the optical flow Stage 1 checkpoint (rank16, all layers).
    # IMPORTANT: this must match the architecture in standard_meta_model_zero_config.py
    #   lora_rank=16, adaptor_layers=list(range(12)), use_flow=True
    ...
    train_ds_params = {
        ...
        "ds_params": {
            ...
            "isValid":        False,
            # Flow must be loaded in Stage 2 so the flow branch receives
            # real flow tensors during translation training
    ...
    # ── Load Stage 1 config for model architecture ────────────────────────────
    # Stage 2 needs the Stage 1 model architecture (sign encoder) to rebuild
    # the model. It uses the config to get the architecture params, then loads
    # weights from stage1_ckpt below.
    cfg.stage1_name = (
    ...
    # ── Stage 1 checkpoint for sign encoder weights ───────────────────────────
    # FIXED: hardcoded path to the correct checkpoint.
    # Previously used get_best_checkpoint_details() which found the wrong
    # checkpoint (rank=4, top 3 layers) causing size mismatch errors.
    # This checkpoint matches the current architecture:
    #   - lora_rank=16, adaptor_layers=list(range(12)), use_flow=True
    # stage1_ckpt = "dinov3_optical_flow_rank16_alllayers_v1/phoenix_stage1_configs/PHX_example_s1_dyn_config/best_result_checkpoint_73_0.1799.pt"
# 
